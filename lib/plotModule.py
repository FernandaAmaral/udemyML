...
class TreePlot:
    ...
    def plot(self, classifier, img_name, feature_labels, class_labels):
        dot_data = StringIO()
...
class AUC:
    ...
    def plot(self, classifier, X_train, y_train, X_test, y_test, fig, title, subplot_index):
        ...
        y_train_proba = pd.DataFrame(
            classifier.predict_proba(X_train))[1].tolist()
        y_test_proba = pd.DataFrame(
            classifier.predict_proba(X_test))[1].tolist()
        ...
        # legend
        legenda = plt.legend(
            ['treino: %.2f' % y_train_auc, 'teste: %.2f' % y_test_auc, 'no skill: 0.50'])
        renderizador = fig.canvas.get_renderer()
        shift = max([t.get_window_extent(
            renderizador).width for t in legenda.get_texts()])
